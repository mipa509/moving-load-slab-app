...
- **Next action:** independent numerical review of WP-026, then **WP-032A** (solver-facade transport: expose `s/t`, polygon patches, `mxy`, reaction coords, equilibrium/quality/verification through `src/solver/index.ts`) followed by WP-032B/WP-032C and WP-027 (app `EdgeSupport` union). The non-zero-skew experimental warning is emitted from the solver (`warnings[]`) and stays until G7.
